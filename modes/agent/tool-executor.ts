...
const WINDOWS_INVALID_CHARS_RE = /[<>:"|?*\x00-\x1f]/;

function assertValidWindowsPath(relPath: string): void {
  const segments = relPath.split(/[\\/]/).filter(Boolean);
  for (const seg of segments) {
    if (WINDOWS_INVALID_CHARS_RE.test(seg)) {
      throw new Error(`Invalid path segment for Windows: "${seg}" in ${relPath}`);
    }
    const bare = seg.replace(/\.[^.]*$/, "");
    if (WINDOWS_RESERVED_NAMES.has(bare.toUpperCase())) {
      throw new Error(`Path segment is a reserved Windows device name: "${seg}" in ${relPath}`);
    }
    if (/[ .]$/.test(seg)) {
      throw new Error(`Path segment cannot end with a space or dot on Windows: "${seg}" in ${relPath}`);
    }
  }
}
...
export class ToolExecutor {
  ...
  private resolveSafe(rel: string): string {
    ...
    // Text-based check above can be defeated by a symlink inside the
    // workspace that points outside it. Resolve the real (symlink-free)
    // path of the nearest existing ancestor and re-check against it.
    ...
    const realAbs = tail.length ? path.join(realDir, ...tail) : realDir;
    const realRelCheck = path.relative(realRoot, realAbs);
  ...
  private excluded(relPath: string): boolean {
    ...
    for (const patRaw of this.config.excludePatterns) {
      const pat = patRaw.replace(/\\/g, "/").toLowerCase();
      if (pat.includes("*") || pat.includes("?")) {
        const re = globToRegExp(pat);
        if (re.test(base) || re.test(norm) || segments.some((s) => re.test(s)))
          return true;
        continue;
      }
  ...
  searchFiles(
    ...
    const nameRe = globToRegExp(globPattern.replace(/\\/g, "/"));
  ...
  applyApprovedFromTracker(): { errors: string[] } {
    ...
    for (const a of all.filter(
      ...
      if (r.error) errors.push(`shell error: ${cmd}: ${r.error.message}`);
